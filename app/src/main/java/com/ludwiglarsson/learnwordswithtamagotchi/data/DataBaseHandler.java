...
public class DataBaseHandler extends SQLiteOpenHelper {
    ...
    public void addWord(Words word, String tableName) {
        ...
        switch (tableName) {
            ...
            case "animals":
                ...
                break;
            case "space": db.insert(Util.TABLE3_NAME, null, contentValues);
                break;
            case "architecture": db.insert(Util.TABLE4_NAME, null, contentValues);
                break;
            case "professions": db.insert(Util.TABLE5_NAME, null, contentValues);
                break;
            case "inventions": db.insert(Util.TABLE6_NAME, null, contentValues);
                break;
        }
        db.close();
    }
    ...
    public Words getWord(int id, String tableName) {
        ...
        while (cursor != null && cursor.moveToNext()) {
            Words words = new Words(Integer.parseInt(cursor.getString(0)),
                    cursor.getString(1), cursor.getString(2), cursor.getString(3), cursor.getString(5));
            cursor.close();
            return words;
        }
        if (cursor != null) {
    ...
    public void deleteWord(int id, String tableName) {
        ...
        switch (tableName) {
            case "words":
                ...
                break;
            case "plants":
                db.delete(Util.TABLE1_NAME, Util.KEY_ID + "=?", new String[]{String.valueOf(id)});
                break;
            case "animals":
                db.delete(Util.TABLE2_NAME, Util.KEY_ID + "=?", new String[]{String.valueOf(id)});
                break;
            case "space":
                db.delete(Util.TABLE3_NAME, Util.KEY_ID + "=?", new String[]{String.valueOf(id)});
                break;
            case "architecture":
                db.delete(Util.TABLE4_NAME, Util.KEY_ID + "=?", new String[]{String.valueOf(id)});
                break;
            case "professions":
                db.delete(Util.TABLE5_NAME, Util.KEY_ID + "=?", new String[]{String.valueOf(id)});
                break;
            case "inventions":
                db.delete(Util.TABLE6_NAME, Util.KEY_ID + "=?", new String[]{String.valueOf(id)});
                break;
        }
        db.close();
    }
    ...
    public Words getRandomWord(String tableName) {
        ...
        return getWord(randomNum, tableName);
    }
